...
const DEFAULT_IMAGE = process.env.PUBLIC_URL + 'house.png'; // Ảnh mặc định khi không có media
            ...
            return (
              <div key={post.id} className="border rounded shadow p-4 flex flex-row w-full gap-[0.8rem]">
                <div>
                  <a href={`/post/${post.id}`}>
                    <img
                      src={firstMedia}
                      alt={room.room_name}
                      className="w-full h-[10rem] object-cover rounded mb-2"
                ...
                <div>
                  ...
                  <p className="text-[#cccccc] text-[0.8rem] mb-[0.5rem]">Đăng lúc: {post.created_at}  |  Cập nhật: {post.updated_at}</p>
